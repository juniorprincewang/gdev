...
*/
	...
	printf("cuMemAlloc (b) dev addr 0x%llx\n", b_dev);
	/* c[] */
	...
	printf("cuMemAlloc (c) dev addr 0x%llx\n", c_dev);
    ...
    if(flag_pte) {
    	for (i = 0; i < n; i++) {
    		...
    		for(j = 0; j < n; j++) {
                ...
                dummy_c = a[idx] - c[idx];
    			if(dummy_c) {
	    		    printf("Failed PTE: a[%d]=%d\tb[%d]=%d\tc[%d]=%d\n",
                            idx, a[idx], idx, b[idx], idx, c[idx]);
			    	break;
    			}
	    		idx++;
    	...
    	for (i = 0; i < n; i++) {
    		...
    		for(j = 0; j < n; j++) {
    			...
    			if(dummy_c) {
	    		    printf("Failed : a[%d]=%d\tb[%d]=%d\tc[%d]=%d\n",
                            idx, a[idx], idx, b[idx], idx, c[idx]);
			    	break;
    			}
	    		idx++;
